...
Weather_Data_Missing_Values <- Weather_Data_Missing_Values %>%
  ...
  transform(resultspeed = convert_wind_speed(resultspeed, old_metric = "M", new_metric = "K")) %>%
...
Weather_Data_Missing_Values_temp <- Weather_Data_Missing_Values[1:50, ]
...
#~~~~~~~~~~~~
# Replace NA's with -1.0 set columns to numeric then rechange back to NA's 
#~~~~~~~~~~~
Weather_Data_knn$snowfall <- replace(Weather_Data_knn$snowfall, is.na(Weather_Data_knn$snowfall), -1.0)
Weather_Data_knn$preciptotal <- replace(Weather_Data_knn$preciptotal, is.na(Weather_Data_knn$preciptotal), -1.0)
Weather_Data_knn$preciptotal <- as.numeric(Weather_Data_knn$preciptotal)
Weather_Data_knn$snowfall <- as.numeric(Weather_Data_knn$snowfall)
Weather_Data_knn$snowfall <- replace(Weather_Data_knn$snowfall, Weather_Data_knn$snowfall == -1.0, "NA")
Weather_Data_knn$preciptotal <- replace(Weather_Data_knn$preciptotal, Weather_Data_knn$preciptotal == -1.0, "NA")

#~~~~~~~~~~~~
# Change codesum from factors to character then to numeric 
#~~~~~~~~~~~
Weather_Data_knn$codesum <- as.list(Weather_Data_knn$codesum)
Weather_Data_knn$codesum <- as.numeric(Weather_Data_knn$codesum)

#~~~~~~~~~~~~
# Remove date and station_nbr before knn 
#~~~~~~~~~~~
Weather_Data_DS <- Weather_Data_knn %>%
  select(station_nbr, date)

Weather_Data_Minus_DS <- Weather_Data_knn %>%
  select(tmax, tmin, tavg, depart, dewpoint, wetbulb, heat, cool,       
         sunrise, sunset, codesum, snowfall, preciptotal,
         stnpressure, sealevel, resultspeed, resultdir, avgspeed)

write.table(Weather_Data_knn, file = "data/technically_correct2/weather_data.csv", append = FALSE, quote = TRUE, sep = ",",
            eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE,
            qmethod = c("escape", "double"), fileEncoding = "")

csv <-"data/technically_correct2/weather_data.csv"
Weather_Data_knn <- read.csv(csv, header = TRUE, sep = ",", dec = ".")
...
Weather_Data_Complex$codesum <- rescale(Weather_Data$codesum)
...
write.table(Weather_Data_Complex, file = "data/aggregated_formatted4/weather_data_rescaled.csv", append = FALSE, quote = TRUE, sep = ",",
            eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE,
            qmethod = c("escape", "double"), fileEncoding = "")
