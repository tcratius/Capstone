...
#~~~~~~~~~~~
model_one <- lm(chron(sunrise) ~ station_nbr + date, data = Weather_Data)
I <- is.na(Weather_Data$sunrise)
Weather_Data$sunrise[I] <- predict(model_one, newdata = Weather_Data[I, ])
...
Weather_Data_Sunset_Cool <- Weather_Data %>%
  ...
  filter(date <= as.Date("2014-06-30")) %>%
